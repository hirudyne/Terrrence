...
ENHANCE_URL = "http://purpose-voxpop:8002"


class EnhanceLineBody(BaseModel):
    line_id: str
    line_index: int
...
async def enhance_line(
    ...
    """Receive a recorded WAV, run through Resemble Enhance, save as line audio asset."""
    ...
    # Send to enhance service
    async with _httpx.AsyncClient(timeout=120) as client:
        resp = await client.post(
            f"{ENHANCE_URL}/enhance",
            files={"file": ("input.wav", wav_bytes, "audio/wav")},
        )
    if resp.status_code != 200:
        raise HTTPException(status_code=502, detail=f"enhance: {resp.text[:200]}")

    enhanced_bytes = resp.content
    sha256 = _hashlib.sha256(enhanced_bytes).hexdigest()

    # Save to assets
    assets_dir = PROJECTS_ROOT / project_slug / "assets"
    assets_dir.mkdir(exist_ok=True)
    filename = f"recorded_{entity_slug}_{line_id}_{line_index}_{sha256[:8]}.wav"
    (assets_dir / filename).write_bytes(enhanced_bytes)
    rel_path = f"assets/{filename}"
    ...
    with db() as conn:
        ...
        if existing:
            ...
            cur = conn.execute(
                "INSERT INTO assets (project_id, rel_path, mime, bytes, sha256) VALUES (?,?,?,?,?)",
                (project["id"], rel_path, "audio/wav", len(enhanced_bytes), sha256),
            )
            asset_id = cur.lastrowid
    ...
    def _patch_line(items: list, target_id: str, idx: int) -> bool:
        for item in items:
            ...
            if item.get("id") == target_id:
                ...
                if 0 <= idx < len(lines_list):
                    lines_list[idx]["audio"] = asset_id
                    return True
            if _patch_line(item.get("response_menu", []), target_id, idx):
                return True
        return False

    patched = False
    for greeting in conv_data.get("greetings", []):
        if isinstance(greeting, dict) and greeting.get("id") == line_id:
            lines_list = greeting.get("lines", [])
            if 0 <= line_index < len(lines_list):
                lines_list[line_index]["audio"] = asset_id
                patched = True
                break
    if not patched:
        _patch_line(conv_data.get("menu", []), line_id, line_index)
